...
import (
	...
	"github.com/dselans/billing-svc/types"
)

const (
	BucketName = "billing-svc"
)
...
func (i *ISB) handleSignupEvent(event *types.SignupEvent, eventData []byte) {
	...
	if isKnown {
		i.log.Debugf("billing: email '%s' is known - ignoring", event.Email)
	} else {
		i.log.Debugf("billing: email '%s' is unknown - creating stripe profile", event.Email)
	}
}
